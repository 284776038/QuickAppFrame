...
public abstract class BaseWebView extends WebView {
    ...
    private static final String APP_CACAHE_DIRNAME = "/rodo_webcache";//文件缓存目录

    private static final String USER_AGENT_1 = "rodo";          //标识是否在wentianji应用内
    private static final String USER_AGENT_DOWN_4_2 = "rodo_native_down_4.2"; //标识是否带有Android原生接口;是：“”，否
	...
	public void initWebView(Context context) {
		...
		if(Build.VERSION.SDK_INT >= 19) {
			try {
				...
				LogUtils.e(TAG,"NoSuchMethodError_setWebContentsDebuggingEnabled");
			}
		}
...
//		}
    ...
    public void loadUrl(String url, Map<String, String> extraHeaders) {
        if(isDestory){
        	if(DEBUG){ LogUtils.w(TAG, "webview已经销毁，无法执行该操作"); }
            return ;
        }
	...
	public void callJS(String callback,String data,String errorCode){
		if(TextUtils.isEmpty(callback)){
			if(DEBUG){ LogUtils.w(TAG, "callJS callback is isEmpty！"+callback); }
    		return;
    	}
    	JSONObject json = null;
    	String callJSData = null;
    	if(DEBUG){ LogUtils.d(TAG, "callJS: "+data); }
    	...
    	if(DEBUG){ LogUtils.i(TAG, "callJS:"+callJSData); }
	...
	private void callJS(String javaScriptData){
		if(isDestory){
			if(DEBUG){ LogUtils.e(TAG, "webview已经销毁，callJS 无法执行该操作 "+javaScriptData); }
			return;
        }
